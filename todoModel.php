...
function addJob($name, $number,$father,$mother,$type) {
	global $conn;
	$sql = "insert into apply (name, number, father,mother,type,status) values ('$name', '$number','$father','$mother','$type',0);"; //新增一筆資料
...
{
	global $conn;
	$sql = "update apply set teacher_comment = '$teacher_comment', teacher_name = '$teacher_name' where id=$id;";
	mysqli_query($conn, $sql) or die("Insert failed, SQL query error"); //執行SQL
}
...
{
	global $conn;
	$sql = "update apply set result = '$result',  secretary_comment = '$secretary_comment', secretary_name = '$secretary_name' where id=$id;";
	mysqli_query($conn, $sql) or die("Insert failed, SQL query error"); //執行SQL
}
...
function setFinished($id) { //將工作設為已完成(0-->1)
	global $conn;
	$sql = "update apply set status = 1  where id=$id and status = 0;";
...
function rejectJob($id){ //撤回工作，從已完成改成未完成(1-->0)
	global $conn;
	$sql = "update apply set status = 0 where id=$id and status = 1;";
	mysqli_query($conn,$sql);
}
